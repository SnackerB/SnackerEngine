...
namespace SnackerEngine
{
	...
	struct Mat4
	{
		...
		static Mat4<T> PerspectiveProjection(const T& fovy, const T& aspect, const T& near, const T& far);
	...
	//------------------------------------------------------------------------------------------------------
	template<typename T>
	inline Mat4<T> Mat4<T>::PerspectiveProjection(const T& fovy, const T& aspect, const T& near, const T& far)
	{
		T ta = tan(fovy / T(2));
		return Mat4<T>({
			T(1) / (ta * aspect), T(0), T(0), T(0),
			T(0), T(1) / ta, T(0), T(0),
