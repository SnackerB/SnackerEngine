...
namespace SnackerEngine
{
	...
	void SERPManager::handleReceivedMessage(SnackerNet::HTTPMessage& message)
	{
		switch (message.getMessageType())
		{
		case SnackerNet::HTTPMessage::MessageType::REQUEST:
		{
			auto serpRequest = SnackerNet::SERPRequest::parse(static_cast<SnackerNet::HTTPRequest&>(message));
			if (serpRequest.has_value()) {
				...
				errorLogger << LOGGER::BEGIN << "obtained HTTP request does not follow SERP format:\"\n" << 
					static_cast<SnackerNet::HTTPRequest&>(message).toString() << "\n\"" << LOGGER::ENDL;
			}
			break;
		}
		case SnackerNet::HTTPMessage::MessageType::RESPONSE:
		{
			auto serpResponse = SnackerNet::SERPResponse::parse(static_cast<SnackerNet::HTTPResponse&>(message));
			if (serpResponse.has_value()) {
				...
				errorLogger << LOGGER::BEGIN << "obtained HTTP response does not follow SERP format:\"\n" <<
					static_cast<SnackerNet::HTTPResponse&>(message).toString() << "\n\"" << LOGGER::ENDL;
			}
			break;
		}
		default:
	...
	void SERPManager::handleIncomingRequest(SnackerNet::SERPRequest& request)
	{
		if (request.getDestination() != serpID) {
			...
			sendResponse(SnackerNet::SERPResponse(serpID, request.getSource(),
				SnackerNet::HTTPResponse{ SnackerNet::ResponseStatusCode::BAD_REQUEST, {}, "destinationID did not match serpID!" }));
		}
		else {
	...
	void SERPManager::handleIncomingManagerRequest(SnackerNet::SERPRequest& request)
	{
		if (request.request.requestMethod == request.request.HTTP_GET && request.request.path == "/ping") {
			...
			sendResponse(SnackerNet::SERPResponse(serpID, request.getSource(),
				SnackerNet::HTTPResponse{ SnackerNet::ResponseStatusCode::NOT_FOUND, {}, "path" + request.request.path + "was not found!" }));
		}
	}
	...
	SERPManager::SERPManager()
		: connected{ false }, serpID{ 0 }, httpEndpoint(SnackerNet::SocketTCP{}, false), incomingRequests{}, pendingResponses{}, outgoingRequests{},
		serpIDResponse{ nullptr }, incomingMessageBuffer{ 4000 }
	{
		...
		httpEndpoint = SnackerNet::HTTPEndpoint(std::move(result.value()), false);
	}
	...
	{
		bool result = SnackerNet::connectTo(httpEndpoint.getSocket(), SnackerNet::getSERPServerAdressTCP());
		if (result) {
			connected = true;
			...
			if (!SnackerNet::setToNonBlocking(httpEndpoint.getSocket())) {
				errorLogger << "Was not able to set socket to non-blocking!" << LOGGER::ENDL;
				httpEndpoint.setSocket(SnackerNet::SocketTCP{});
				return false;
			}
			return true;
		}
	...
	SERPManager::PendingResponse SERPManager::sendRequest(SnackerNet::SERPRequest request, double timeout)
	{
		...
		if (it == pendingResponses.end()) {
			...
			if (it2 == outgoingRequests.end()) {
				...
				auto result = outgoingRequests.insert(std::make_pair<>(request.getDestination(), std::vector<std::pair<SnackerNet::SERPRequest, PendingResponse*>>()));
		...
		if (serpID == 0) {
			if (!serpIDResponse || serpIDResponse->getStatus() == PendingResponse::Status::TIMEOUT) {
				...
				if (response.response.responseStatusCode == SnackerNet::ResponseStatusCode::OK) {
					auto result = SnackerNet::parseSERPID(response.response.body);
					if (result.has_value()) {
						serpID = result.value();
					}
				}
