...
namespace SnackerEngine
{
	...
	static Buffer copyBytes(const std::vector<std::byte>::const_iterator& begin, const std::vector<std::byte>::const_iterator& end, std::size_t offset = 0, std::size_t n = SIZE_MAX)
	{
		...
		memcpy(&(newData[0]), &(*(begin + offset)), newData.size());
	...
	static bool compare(const std::vector<std::byte>::const_iterator& begin, const std::vector<std::byte>::const_iterator& end, const std::string& string)
	{
		...
		for (std::size_t i = 0; i < (end - begin); ++i) {
			if (*(begin + i) != static_cast<std::byte>(string[i])) return false;
		}
		return true;
	}
	...
	{
		if (size == SIZE_MAX) return BufferView(_begin + offset, _end);
		return BufferView(_begin + offset, std::min(_begin + offset + size, _end));
	}

	ConstantBufferView BufferView::getBufferView(std::size_t offset, std::size_t size) const
	{
		if (size == SIZE_MAX) return BufferView(_begin + offset, _end);
		return ConstantBufferView(_begin + offset, std::min(_begin + offset + size, _end));
	}
	...
	{
		if (size == SIZE_MAX) return ConstantBufferView(_begin + offset, _end);
		return ConstantBufferView(_begin + offset, std::min(_begin + offset + size, _end));
	}
	...
	{
		if (size == SIZE_MAX) return BufferView(data.begin() + offset, data.end());
		return BufferView(data.begin() + offset, std::min(data.begin() + offset + size, data.end()));
	}

	ConstantBufferView Buffer::getBufferView(std::size_t offset, std::size_t size) const
	{
		if (size == SIZE_MAX) return ConstantBufferView(data.begin() + offset, data.end());
		return ConstantBufferView(data.begin() + offset, std::min(data.begin() + offset + size, data.end()));
	}
